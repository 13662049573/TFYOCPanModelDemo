...
//
//  TFYPanModalPresentableHandler.m
//  TFYPanModal
//
...
@interface TFYPanModalPresentableHandler ()
...
@property (nonatomic, weak) id<TFYPanModalPresentable, TFYPanModalPanGestureDelegate> presentable;
...
// =====================
//
// 1. 构造方法、工厂方法、手势处理、KVO、状态切换等关键方法补充中文注释
// 2. 重要属性、内部状态、回调点补充注释
// 3. KVO相关增加NSAssert保护，避免提前释放导致crash
// 4. 关键分支、易混淆逻辑增加注释说明
//
- (instancetype)initWithPresentable:(id<TFYPanModalPresentable, TFYPanModalPanGestureDelegate>)presentable {
    self = [super init];
    if (self) {
...
+ (instancetype)handlerWithPresentable:(id<TFYPanModalPresentable, TFYPanModalPanGestureDelegate>)presentable {
...
- (BOOL)shouldHandleShortStatePullDownWithRecognizer:(UIPanGestureRecognizer *)recognizer {
    ...
    if ((self.presentedView.frame.origin.y >= self.shortFormYPosition || TFY_TWO_FLOAT_IS_EQUAL(self.presentedView.frame.origin.y, self.shortFormYPosition)) && location.y > 0) {
        return YES;
    }
...
- (void)respondToPanGestureRecognizer:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    id <TFYPanModalPresentableHandlerDelegate> delegate = self.delegate;
...
- (CGFloat)nearestDistance:(CGFloat)position inDistances:(NSArray *)distances {
    ...
    // 优化：一次遍历找最小差值，避免多余排序和字典操作，提升效率
    CGFloat minDiff = CGFLOAT_MAX;
    NSNumber *nearest = nil;
    for (NSNumber *number in distances) {
        CGFloat diff = fabs(number.floatValue - position);
        if (diff < minDiff) {
            minDiff = diff;
            nearest = number;
        }
    }
    return nearest ? nearest.floatValue : position;
}
...
- (void)handlePanGestureBeginOrChanged:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    if (panGestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if ([[self presentable] presentingVCAnimationStyle] > PresentingViewControllerAnimationStyleNone &&
                velocity.y > 0 &&
                (self.presentedView.frame.origin.y > self.shortFormYPosition || TFY_TWO_FLOAT_IS_EQUAL(self.presentedView.frame.origin.y, self.shortFormYPosition))) {
            [self dismissPresentable:YES mode:PanModalInteractiveModeDragDown];
        }
    }

    if (TFY_TWO_FLOAT_IS_EQUAL(self.presentedView.frame.origin.y, self.anchoredYPosition) && self.extendsPanScrolling) {
    ...
    if (panGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        if (velocity.y > 0) {
            [self.dragIndicatorView didChangeToState:TFYIndicatorStatePullDown];
...
- (void)handlePanGestureEnded:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    if ([self isVelocityWithinSensitivityRange:velocity.y]) {
        ...
        if (TFY_TWO_FLOAT_IS_EQUAL(position, self.longFormYPosition)) {
            [self transitionToState:PresentationStateLong];
            [self cancelInteractiveTransition];
        } else if (TFY_TWO_FLOAT_IS_EQUAL(position, self.mediumFormYPosition)) {
            [self transitionToState:PresentationStateMedium];
            [self cancelInteractiveTransition];
        } else if (TFY_TWO_FLOAT_IS_EQUAL(position, self.shortFormYPosition) || ![self.presentable allowsDragToDismiss]) {
...
- (void)observeScrollable {
    ...
    if (!scrollView) {
        ...
        if ([self.observerToken respondsToSelector:@selector(unObserver)]) {
...
- (void)haltScrolling:(UIScrollView *)scrollView {
    ...
    // See issues: https://github.com/HeathWang/TFYPanModal/issues/107 and https://github.com/HeathWang/TFYPanModal/issues/103
...
- (void)configureScrollViewInsets {
    ...
    if ([self.presentable panScrollable] && ![self.presentable panScrollable].isScrolling) {
        ...
        CGFloat bottomLayoutOffset = 0;
        if (@available(iOS 11.0, *)) {
            UIWindowScene *windowScene = (UIWindowScene *)[UIApplication sharedApplication].connectedScenes.allObjects.firstObject;
            UIWindow *window = windowScene.windows.firstObject;
            UIViewController *rootVC = window.rootViewController;
            bottomLayoutOffset = rootVC.view.safeAreaInsets.bottom;
        }
        if (TFY_FLOAT_IS_ZERO(insets1.bottom) || insets1.bottom < bottomLayoutOffset) {
...
- (void)setScrollableContentOffset:(CGPoint)offset animated:(BOOL)animated {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) ((animated ? 0.30 : 0.1) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf trackScrolling:scrollView];
        [weakSelf observeScrollable];
    });
...
- (void)configureViewLayout {
    ...
    if ([self.presentable isKindOfClass:UIViewController.class]) {
        UIViewController<TFYPanModalPresentable> *layoutPresentable = (UIViewController<TFYPanModalPresentable> *) self.presentable;
    ...
    } else if ([self.presentable isKindOfClass:TFYPanModalContentView.class]) {
        TFYPanModalContentView<TFYPanModalPresentable> *layoutPresentable = (TFYPanModalContentView<TFYPanModalPresentable> *) self.presentable;
        self.shortFormYPosition = layoutPresentable.shortFormYPos;
        self.mediumFormYPosition = layoutPresentable.mediumFormYPos;
        self.longFormYPosition = layoutPresentable.longFormYPos;
        self.anchorModalToLongForm = [layoutPresentable anchorModalToLongForm];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    ...
    if ([self.presentable conformsToProtocol:@protocol(TFYPanModalPanGestureDelegate)] &&
        [self.presentable respondsToSelector:@selector(panGestureRecognizer:shouldRecognizeSimultaneouslyWithGestureRecognizer:)]) {
        return [self.presentable panGestureRecognizer:gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:otherGestureRecognizer];
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldBeRequiredToFailByGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    ...
    if ([self.presentable conformsToProtocol:@protocol(TFYPanModalPanGestureDelegate)] &&
        [self.presentable respondsToSelector:@selector(panGestureRecognizer:shouldBeRequiredToFailByGestureRecognizer:)]) {
        return [self.presentable panGestureRecognizer:gestureRecognizer shouldBeRequiredToFailByGestureRecognizer:otherGestureRecognizer];
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    if ([self.presentable conformsToProtocol:@protocol(TFYPanModalPanGestureDelegate)] &&
        [self.presentable respondsToSelector:@selector(panGestureRecognizer:shouldRequireFailureOfGestureRecognizer:)]) {
        return [self.presentable panGestureRecognizer:gestureRecognizer shouldRequireFailureOfGestureRecognizer:otherGestureRecognizer];
    }
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if ([self.presentable conformsToProtocol:@protocol(TFYPanModalPanGestureDelegate)] &&
        [self.presentable respondsToSelector:@selector(panGestureRecognizerShouldBegin:)]) {
        return [self.presentable panGestureRecognizerShouldBegin:gestureRecognizer];
    }
    ...
    if (gestureRecognizer == self.screenEdgeGestureRecognizer) {
        ...
        if (velocity.x <= 0 || TFY_TWO_FLOAT_IS_EQUAL(velocity.x, 0)) {
            return NO;
        }
        ...
        if (velocity.x > 0 && TFY_TWO_FLOAT_IS_EQUAL(velocity.y, 0)) {
            return YES;
        }

        //TODO: this logic can be updated later.
        // 优化建议：此处逻辑可根据后续需求进行扩展和优化，当前为默认实现。
